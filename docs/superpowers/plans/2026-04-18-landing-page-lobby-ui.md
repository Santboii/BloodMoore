# Landing Page & Lobby UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the existing minimal LobbyUI with a full Blood Moor–themed landing page (lobby browser, mode selector, join-by-code) and a lobby waiting room with player slots, ready-up, and pre-game chat.

**Architecture:** Server gains a `GET /rooms` list endpoint and a `chat-message` socket event; client rewrites `LobbyUI.ts` with a persistent Blood Moor SVG background and swappable screen content, extends `SocketClient.ts` with chat helpers, and updates `main.ts` to pass the new signatures.

**Tech Stack:** Node.js / Express / Socket.io (server), TypeScript / Vite / Socket.io-client (client), Vitest (server tests).
...
|---|---|---|
| `server/src/rooms/Room.ts` | Modify | Add `creatorName: string` property |
| `server/src/rooms/RoomManager.ts` | Modify | Add `listOpenRooms()` method |
| `server/src/index.ts` | Modify | Add `GET /rooms` endpoint + `chat-message` socket event |
| `server/tests/room.test.ts` | Create | Tests for `creatorName` |
| `server/tests/roommanager.test.ts` | Create | Tests for `listOpenRooms()` |
| `client/src/network/SocketClient.ts` | Modify | Add `sendChatMessage` + `onChatMessage` |
| `client/src/lobby/LobbyUI.ts` | Rewrite | Full Blood Moor UI (all screens) |
| `client/src/main.ts` | Modify | Updated call sites + chat wiring |

---

## Task 1: Add `creatorName` to `Room`
...
```bash
cd server && npm test -- --reporter=verbose 2>&1 | grep -A3 'creatorName'
```

Expected: `TypeError: room.creatorName is not defined` or similar.

- [ ] **Step 3: Add `creatorName` to `Room.ts`**

```typescript
// server/src/rooms/Room.ts  — full file
...
git commit -m "feat: add creatorName to Room — stored on first player join"
```

---

## Task 2: Add `listOpenRooms()` to `RoomManager`
...
```bash
cd server && npm test -- --reporter=verbose 2>&1 | grep -A3 'listOpenRooms'
```

Expected: `TypeError: rm.listOpenRooms is not a function`.

- [ ] **Step 3: Add `listOpenRooms()` to `RoomManager.ts`**

```typescript
// server/src/rooms/RoomManager.ts  — full file
...
git commit -m "feat: add listOpenRooms to RoomManager — returns non-full, not-started rooms"
```

---

## Task 3: Add `GET /rooms` endpoint + `chat-message` socket event

**Files:**
- Modify: `server/src/index.ts`

- [ ] **Step 1: Add `GET /rooms` endpoint and `chat-message` handler to `index.ts`**
...
Note: `player-ready` now emits `player-ready-ack` to all room members so the client can update ready status indicators.

- [ ] **Step 2: Start server and verify `GET /rooms` responds**

```bash
cd server && npm run dev &
sleep 2
curl -s http://localhost:3000/rooms | python3 -m json.tool
```
...
- [ ] **Step 3: Run all server tests — expect all pass**
...
git commit -m "feat: add GET /rooms endpoint and chat-message socket event"
```

---

## Task 4: Extend `SocketClient` with chat + ready-ack

**Files:**
- Modify: `client/src/network/SocketClient.ts`

- [ ] **Step 1: Add `sendChatMessage`, `onChatMessage`, and `onPlayerReadyAck` to `SocketClient.ts`**
...
git commit -m "feat: add sendChatMessage, onChatMessage, onPlayerReadyAck to SocketClient"
```

---

## Task 5: Rewrite `LobbyUI.ts`

**Files:**
- Rewrite: `client/src/lobby/LobbyUI.ts`

- [ ] **Step 1: Write the complete new `LobbyUI.ts`**
...
export class LobbyUI {
  ...
  private getSenderColorClass(senderId: string): string {
    // Cycle through two colours based on a stable hash of the id
...
git commit -m "feat: rewrite LobbyUI — Blood Moor landing page, lobby screen with chat, Diablo aesthetic"
```

---

## Task 6: Update `main.ts`

**Files:**
- Modify: `client/src/main.ts`

- [ ] **Step 1: Replace `main.ts` with the updated version**

```typescript
// client/src/main.ts  — full file
...
function setupSocketHandlers(): void {
  ...
  socket.onChatMessage(({ senderId, displayName: senderName, text }) => {
    lobby.appendChatMessage(senderId, senderName, text);
  });

  socket.onPlayerJoined(({ id, displayName: oppName }) => {
    opponentName = oppName;
    const players = { [myId]: myDisplayName, [id]: oppName };
    lobby.showReady(currentRoomId, players, myId);
    lobby.appendSystemMessage(`${oppName} has entered the lobby`);
  });

  socket.onGameReady(() => { /* both players connected — lobby already shows ready screen */ });
...
scene.startRenderLoop(() => {
  ...
  if (myPlayer) scene.updateCamera(myPlayer.position.x, myPlayer.position.y, delta);
...
git commit -m "feat: wire new LobbyUI signatures, chat events, and currentRoomId tracking in main.ts"
```
...
Checklist:
- Blood Moor background renders (dark sky, trees, fog)
- "Blood Moor" title with glow effect
- Name input, 1v1 mode selected, 2v2/3v3/FFA locked with "Soon" badge
- "Create Lobby" red button, join-by-code input + button
- Open Lobbies panel with pulsing green dot (empty state initially)

- [ ] **Step 3: Verify lobby creation + browser**

1. Open two browser tabs to `http://localhost:5173`
2. In Tab 1: type name "Grimshaw" → click "Create Lobby"
3. Expected: Tab 1 shows the Lobby screen with room code, 1 player slot filled, disabled Ready button
4. In Tab 2: refresh the landing page — the lobby browser should show "Grimshaw · 1/2 · Join"
5. Tab 2: type name "Darkbane" → click Join on the room row
6. Expected: Both tabs show 2-player lobby with Ready buttons enabled

- [ ] **Step 4: Verify chat**

1. In Tab 1 lobby: type "gl hf" in War Council input → press Enter
2. Expected: message appears in both tabs with correct sender name and colour

- [ ] **Step 5: Verify ready-up and game start**

1. Both players click "⚔ Ready"
2. Expected: lobby hides, game arena renders with both characters

- [ ] **Step 6: Verify join-by-code**

1. From Tab 1 lobby screen, copy the invite link
2. Open a new tab to that URL
3. Expected: landing page loads with room code pre-filled in "Join by Code" input

- [ ] **Step 7: Final commit**

```bash
git add .
git commit -m "chore: smoke test confirmed — landing page and lobby UI complete"
```
...
- `player-ready-ack` is emitted by the server so clients can update ready indicators in real time (added in Task 3). The current `renderLobby` renders ready state at construction time only; a future improvement could listen to `onPlayerReadyAck` and update the slot status DOM directly without re-rendering. For now the game starts immediately when both are ready, so the gap is imperceptible.
- The `getSenderColorClass` in `LobbyUI` assigns colours by socket ID hash — this is stable within a session but will vary across refreshes. A future improvement (once auth is in place) can assign colours by user ID.
- Add `.superpowers/` to `.gitignore` if not already present to avoid committing brainstorm mockup files.
